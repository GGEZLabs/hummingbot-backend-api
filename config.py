...
class MarketDataSettings(BaseSettings):
    ...
    cleanup_interval: int = Field(default=300, description="How often to run feed cleanup in seconds")
    feed_timeout: int = Field(default=600, description="How long to keep unused feeds alive in seconds")
...
class SecuritySettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="", extra="ignore")  # Ignore extra environment variables
...
class AppSettings(BaseSettings):
    ...
    logfire_environment: str = Field(default="dev", description="Logfire environment name")

    # Account state update interval
    account_update_interval: int = Field(default=5, description="How often to update account states in minutes")
...
class Settings(BaseSettings):
    ...
    banned_tokens: List[str] = Field(default=["NAV", "ARS", "ETHW", "ETHF"], description="List of banned trading tokens")
