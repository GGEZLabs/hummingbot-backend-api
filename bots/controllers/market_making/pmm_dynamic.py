...
class PMMDynamicController(MarketMakingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        number_of_nulls = candles["close"].isnull().sum()
        candles = candles.dropna(subset=["close"])

        required_data_points = max(self.config.macd_fast, self.config.macd_slow) + self.config.macd_signal
        _types=candles.dtypes
