...
from utils.client_config_manager import ClientConfManager
...
class AccountsService:
    ...
    def __init__(
        ...
        self.db_manager = AsyncDatabaseManager(settings.database.url, settings.database.used_schema)
        ...
        self.client_conf_manager = ClientConfManager()  # Placeholder for client config manager if needed
    ...
    async def load_account_state_history(
        self,
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                return await repository.get_account_state_history(
                    limit=limit, cursor=cursor, start_time=start_time, end_time=end_time
                )
        except Exception as e:
    ...
    def _initialize_rate_sources_for_pairs(self, connector_name: str, trading_pairs: List[str]):
        ...
        try:
            connector_pairs = [
                ConnectorPair(connector_name=connector_name, trading_pair=trading_pair) for trading_pair in trading_pairs
            ]
    ...
    async def _initialize_price_tracking(self, account_name: str, connector_name: str, connector):
        ...
        try:
            ...
            unique_tokens = [
                ...
                if value != Decimal("0") and token not in settings.banned_tokens and "USD" not in token
            ]
            ...
            if unique_tokens:
                ...
                logger.info(
                    "Initialized price tracking for"
                    f" {len(trading_pairs)} trading pairs in {connector_name} "
    ...
    async def _get_connector_tokens_info(
        self, connector, connector_name: str, market_data_manager: Optional[MarketDataFeedManager] = None
    ) -> List[Dict]:
        ...
        if trading_pairs_need_update:
            ...
            logger.info(
                f"Added {len(trading_pairs_need_update)} new trading pairs to market data provider: {trading_pairs_need_update}"
            )
    ...
    def list_credentials(account_name: str):
        ...
        """
        try:
            return [file for file in fs_util.list_files(f"credentials/{account_name}/connectors") if file.endswith(".yml")]
    ...
    def add_account(self, account_name: str):
        ...
        files_to_copy = [
            "configs/default/conf_client.yml",
            "configs/default/conf_fee_overrides.yml",
            "configs/default/hummingbot_logs.yml",
            ".password_verification",
        ]
        fs_util.create_folder("credentials", account_name)
        fs_util.create_folder(f"credentials/{account_name}", "connectors")
        fs_util.create_folder(f"credentials/{account_name}", "configs")
    ...
    async def get_account_state_history(
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                repository = AccountRepository(session)
                return await repository.get_account_state_history(
                    account_name=account_name, limit=limit, cursor=cursor, start_time=start_time, end_time=end_time
                )
        except Exception as e:
    ...
    async def get_connector_state_history(
        self,
        account_name: str,
        connector_name: str,
        limit: Optional[int] = None,
                    ...
                    for connector_name, connector_data in self.accounts_state[acc_name].items():
                        for token_info in connector_data:
                            ...
                            if token not in token_values:
                                token_values[token] = {"token": token, "total_value": 0, "total_units": 0, "accounts": {}}
                            ...
                            if acc_name not in token_values[token]["accounts"]:
                                token_values[token]["accounts"][acc_name] = {"value": 0, "units": 0, "connectors": {}}
                            ...
                            if connector_name not in token_values[token]["accounts"][acc_name]["connectors"]:
                                token_values[token]["accounts"][acc_name]["connectors"][connector_name] = {"value": 0, "units": 0}

                            token_values[token]["accounts"][acc_name]["connectors"][connector_name]["value"] += value
                            token_values[token]["accounts"][acc_name]["connectors"][connector_name]["units"] += token_info.get(
                                "units", 0
                            )
    ...
    def get_account_distribution(self) -> Dict[str, any]:
        ...
        try:
            ...
            for acc_name, account_data in self.accounts_state.items():
                ...
                account_values[acc_name] = {"total_value": round(account_value, 6), "connectors": connector_values}
            ...
            for acc_name, acc_data in account_values.items():
                ...
                for conn_name, conn_value in acc_data["connectors"].items():
                    ...
                    connector_dist[conn_name] = {"value": conn_value, "percentage": round(conn_percentage, 4)}
            ...
            return {"total_portfolio_value": 0, "account_count": 0, "distribution": [], "error": str(e)}
    ...
    async def place_trade(
        ...
        market_data_manager: Optional[MarketDataFeedManager] = None,
    ) -> str:
        """
        ...
        if not connector.trading_rules:
            raise HTTPException(
                status_code=503, detail=f"Trading rules not yet loaded for {connector_name}. Please try again in a moment."
            )
        ...
        if order_type not in connector.supported_order_types():
            ...
            raise HTTPException(
                status_code=400, detail=f"Order type '{order_type.name}' not supported. Supported types: {supported_types}"
            )
        ...
        if notional_size < trading_rule.min_notional_size:
            raise HTTPException(
                status_code=400,
                detail=f"Order notional value {notional_size} is below minimum "
                f"notional size {trading_rule.min_notional_size} for {trading_pair}. "
        ...
        try:
            ...
            logger.info(
                f"Placed {trade_type} order for {amount} {trading_pair} "
                f"on {connector_name} (Account: {account_name}). Order ID: {order_id}"
            )
            return order_id
    ...
    async def set_leverage(self, account_name: str, connector_name: str, trading_pair: str, leverage: int) -> Dict[str, str]:
        """
    ...
    async def set_position_mode(self, account_name: str, connector_name: str, position_mode: PositionMode) -> Dict[str, str]:
        """
    ...
    async def get_active_orders_history(
        self, account_name: Optional[str] = None, connector_name: Optional[str] = None, trading_pair: Optional[str] = None
    ) -> List[Dict]:
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                orders = await order_repo.get_active_orders(
                    account_name=account_name, connector_name=connector_name, trading_pair=trading_pair
                )
                return [order_repo.to_dict(order) for order in orders]
        except Exception as e:
    ...
    async def get_orders_summary(
        self, account_name: Optional[str] = None, start_time: Optional[int] = None, end_time: Optional[int] = None
    ) -> Dict:
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                order_repo = OrderRepository(session)
                return await order_repo.get_orders_summary(account_name=account_name, start_time=start_time, end_time=end_time)
        except Exception as e:
    ...
    async def get_funding_payments(
        self, account_name: str, connector_name: str = None, trading_pair: str = None, limit: int = 100
    ) -> List[Dict]:
        """
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                funding_payments = await funding_repo.get_funding_payments(
                    account_name=account_name, connector_name=connector_name, trading_pair=trading_pair, limit=limit
                )
    ...
    async def get_total_funding_fees(self, account_name: str, connector_name: str, trading_pair: str) -> Dict:
        """
        ...
        try:
            ...
            return {"total_funding_fees": 0, "payment_count": 0, "fee_currency": None, "error": str(e)}

    # ----------------- Client Config Management ----------------- #
    def get_account_client_config_by_name(self, account_name: str, config_name: str) -> Dict:
        """
        Get a client config for an account.
        """
        try:
            return self.client_conf_manager.get_client_config_by_name(account_name, config_name)

        except FileNotFoundError as e:
            raise HTTPException(status_code=404, detail=str(e))

    def update_account_client_config_by_name(self, account_name: str, config_name: str, client_config: dict) -> Dict:
        """
        Update a client config for an account.
        """
        try:
            return self.client_conf_manager.update_client_config_by_name(account_name, config_name, client_config)

        except FileNotFoundError as e:
            raise HTTPException(status_code=404, detail=str(e))

    def list_account_configs(self, account_name: str) -> List[str]:
        """
        List all client config folder names for an account.
        """
        try:
            return self.client_conf_manager.list_configs(account_name)

        except FileNotFoundError as e:
            raise HTTPException(status_code=404, detail=str(e))

    def add_new_config_to_account(self, account_name: str, config_name: str) -> None:
        """
        Add a new client config by copying from the default config template.
        """
        try:
            self.client_conf_manager.add_config_folder(account_name, config_name)
    ...
    def delete_config_folder_from_account(self, account_name: str, config_name: str) -> None:
        """
        Delete a client config folder for an account.
        """
        try:
            self.client_conf_manager.delete_config_folder(account_name, config_name)
        except FileNotFoundError as e:
            raise HTTPException(status_code=404, detail=str(e))
