...
from utils.config_encryption import (
    ...
    get_secure_fields_for_controller,
    mask_secure_fields,
)
...
async def list_controllers():
    ...
    for controller_type in ControllerType:
        try:
            ...
            result[controller_type.value] = [f.replace(".py", "") for f in files if f.endswith(".py") and f != "__init__.py"]
        ...
        for config_file in config_files:
            ...
            try:
                ...
                if controller_type and controller_name:
                    secure_fields = get_secure_fields_for_controller(controller_type, controller_name)
                    config = mask_secure_fields(config, secure_fields)

                configs.append(config)
            except Exception as e:
                # If config is malformed, still include it with basic info
                configs.append({"id": config_name, "controller_name": "error", "controller_type": "error", "error": str(e)})
...
async def get_controller_config(config_name: str):
    ...
    Returns:
        Dictionary with controller configuration (secure fields decrypted)
...
async def create_or_update_controller_config(config_name: str, config: Dict):
    ...
    """
    try:
        # Get controller info to load the config class and encrypt secure fields
        controller_type = config.get("controller_type", "")
        controller_name = config.get("controller_name", "")
        if controller_type and controller_name:
            secure_fields = get_secure_fields_for_controller(controller_type, controller_name)
        ...
        logger.error(f"Error saving controller config: {e}")
...
async def get_controller(controller_type: ControllerType, controller_name: str):
    ...
    try:
        ...
        return {"name": controller_name, "type": controller_type.value, "content": content}
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"Controller '{controller_name}' not found in '{controller_type.value}'")
...
async def create_or_update_controller(controller_type: ControllerType, controller_name: str, controller: Controller):
    ...
    if controller.type is not None and controller.type != controller_type:
        raise HTTPException(
            status_code=400, detail=f"Controller type mismatch: URL has '{controller_type}', body has '{controller.type}'"
        )

    try:
        fs_util.add_file(f"controllers/{controller_type.value}", f"{controller_name}.py", controller.content, override=True)
...
async def get_controller_config_template(controller_type: ControllerType, controller_name: str):
    ...
    if config_class is None:
        raise HTTPException(status_code=404, detail=f"Controller configuration class for '{controller_name}' not found")
...
async def update_bot_controller_config(bot_name: str, controller_name: str, config: Dict):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Controller configuration '{controller_name}' not found for bot '{bot_name}'"
        )
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
